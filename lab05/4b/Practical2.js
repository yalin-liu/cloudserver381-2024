...
const formidable = require('formidable');//Alin: require "formidable":"3.5.1"
...
const updateDocument = async (criteria, updateDoc) => {
    await client.connect();
    const db = client.db(dbName);
    let updateContent = { $set : updateDoc };
    const results = await db.collection('bookings').updateOne(criteria, updateContent);
    return results;
}
...
const handle_Update = async (req, res, criteria) => {
    ...
    try {
        ...
        // console.log(fields)
        const DOCID = { '_id': ObjectId.createFromHexString(fields._id[0]) };
        const updateDoc = {
            'bookingid': fields.bookingid[0],
            'mobile': fields.mobile[0]
        };
        
        let fileProps = files.filetoupload[0];
        if (fileProps.size > 0) {
            const data = await fsPromises.readFile(fileProps.filepath);
            updateDoc['photo'] = Buffer.from(data).toString('base64');
        }

        const results = await updateDocument(DOCID, updateDoc);
        res.writeHead(200, { "content-type": "text/html" });
        res.write(`<html><body><p>Updated ${results.modifiedCount} document<p><br>`);
